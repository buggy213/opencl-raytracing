...
impl BVH2Builder {
    ...
    pub fn add_shape(&mut self, shape: &Shape, o2w_transform: &Transform, geom_index: u32) {
        match shape {
            ...
            Shape::Sphere { center, radius } => self.add_sphere(*center, *radius, o2w_transform, geom_index),
        }
    }

    pub fn add_sub_bvh(&mut self, bounds: AABB, o2w_transform: &Transform, geom_index: u32, prim_index: u32) {
...
impl BVH2Builder {
    ...
    fn add_sphere(&mut self, center: Vec3, radius: f32, o2w_transform: &Transform, geom_index: u32) {
        let aabb = AABB::new(
    ...
    fn add_bvh(&mut self, bounds: AABB, o2w_transform: &Transform, geom_index: u32, prim_index: u32) {
            ...
            match node {
                BVHNode::Inner { left_aabb, right_aabb, .. } => {
                    match progress {
                        NodeProgress::None => {
                            ...
                            *progress = NodeProgress::ProcessedLeft { 
                                my_index: index
                            };
                        ...
                        NodeProgress::ProcessedLeft { my_index } => {
                            ...
                            match &mut bvh_nodes[*my_index] {
                                DepthFirstLinearizedBVHNode::Internal { right_child_offset, .. } => 
                ...
                BVHNode::Leaf { prim_count, prim_indices, geom_indices } => {
                    bvh_nodes.push(DepthFirstLinearizedBVHNode::Leaf { 
                        ...
                        prim_count: NonZero::new(*prim_count).expect("prim_count is definitely not zero")
                    });
